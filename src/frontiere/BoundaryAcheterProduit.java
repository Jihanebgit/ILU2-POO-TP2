...
public class BoundaryAcheterProduit {
	...
	public void achatDuProduit(String nomAcheteur) {
		...
		if (!produitPresent) {
			...
			for (int i = 0; i < vendeurs.length; i++) {
				liste.append(i);
				liste.append(" - ");
				liste.append(vendeurs[i]);
				liste.append("\n");
			
			int choix = Clavier.entrerEntier(liste.toString());
			while (choix < 1 || choix > vendeurs.length) {
				choix = Clavier
						.entrerEntier("Veuillez saisir un choix supérieur à 0 et allant jusqu'à " + vendeurs.length);
			}
